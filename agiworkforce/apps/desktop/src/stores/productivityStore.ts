...
export const useProductivityStore = create<ProductivityState>((set, get) => ({
  ...
  connect: async (provider, credentials) => {
    try {
      ...
      const response = await invoke<{ account_id: string }>('productivity_connect', {
        provider,
        credentials,
      });
      ...
      if (provider === 'notion') {
        ...
        // Asana requires workspace_id, so we'll just refresh tasks
  ...
  selectProvider: (provider) => {
    set({ selectedProvider: provider, tasks: [], selectedTaskId: null });
  },
  ...
  // Asana-specific actions
  asanaListProjects: async (workspaceId) => {
    try {
      set({ loading: true, error: null });
      ...
      set({ asanaProjects: projects, loading: false });
    } catch (error) {
