...
export const useEmailStore = create<EmailState>((set, get) => ({
  ...
  removeAccount: async (accountId) => {
    try {
      ...
      if (get().accounts.length > 0) {
        await get().selectAccount(get().accounts[0].id);
      }
    } catch (error) {
  ...
  refreshEmails: async (options) => {
    ...
    try {
      ...
      if (emails.length > 0 && (!get().selectedEmail || get().selectedEmail?.id !== emails[0].id)) {
        set({ selectedEmail: emails[0] });
      }
    } catch (error) {
  ...
  markRead: async (uid, read) => {
    const { selectedAccountId, emails } = get();
    ...
    try {
      ...
      set({
        emails: emails.map((message) => (message.uid === uid ? { ...message, is_read: read } : message)),
        selectedEmail:
          get().selectedEmail && get().selectedEmail?.uid === uid
            ? { ...get().selectedEmail!, is_read: read }
            : get().selectedEmail,
      });
    } catch (error) {
