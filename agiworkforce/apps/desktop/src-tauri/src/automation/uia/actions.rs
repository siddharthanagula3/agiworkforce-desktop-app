...
use crate::automation::uia::patterns::{
    ...
    get_selection_item_pattern, get_table_pattern, get_text_pattern, get_toggle_pattern,
    get_value_pattern,
};
use windows::Win32::UI::Accessibility::{
    ExpandCollapseState_Collapsed, ExpandCollapseState_Expanded, ScrollAmount_LargeDecrement,
    ...
    pub fn scroll_to_element(&self, element_id: &str) -> Result<()> {
        ...
        for _ in 0..10 {
            ...
            if let Some(scroll_pattern) = get_scroll_pattern(&parent) {
                // Get element bounds
                if let Some(bounds) = self.extract_bounds(&element)? {
