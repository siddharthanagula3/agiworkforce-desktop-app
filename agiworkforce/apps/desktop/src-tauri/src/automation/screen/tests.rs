...
mod capture_tests {
    ...
    async fn test_pixel_data_format() {
        ...
        if let Some(pixel) = pixels.get_pixel_checked(0, 0) {
            // Each channel should be 0-255
            assert!(pixel[0] <= 255, "Red channel should be valid");
            assert!(pixel[1] <= 255, "Green channel should be valid");
            assert!(pixel[2] <= 255, "Blue channel should be valid");
            assert!(pixel[3] <= 255, "Alpha channel should be valid");
        }
    }
}
...
mod dxgi_tests {
    ...
    fn test_display_info_populated() {
        ...
        for (i, display) in displays.iter().enumerate() {
            ...
            assert!(!display.name.is_empty(), "Display {} should have a name", i);
        }
    }
