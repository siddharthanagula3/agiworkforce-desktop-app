...
use crate::agent::change_tracker::{ChangeTracker, Change, ChangeType};
...
impl AgentRuntime {
    ...
    pub fn set_auto_approve(&self, enabled: bool) {
        *self.auto_approve.write() = enabled;
        tracing::info!("[AgentRuntime] Auto-approve mode: {}", if enabled { "enabled" } else { "disabled" });
    }
    ...
    pub fn queue_task(&self, mut task: Task) -> Result<String> {
        ...
        tracing::info!("[AgentRuntime] Task queued: {} (priority: {:?})", task_id, task.priority);
    ...
    pub fn get_next_task(&self) -> Option<Task> {
        ...
        let pos = queue.iter().position(|task| {
            task.dependencies.iter().all(|dep_id| {
                completed.iter().any(|t| t.id == *dep_id && t.status == TaskStatus::Completed)
            })
        });
    ...
    pub async fn execute_task(&self, mut task: Task) -> Result<serde_json::Value> {
        ...
        if let Ok(_snapshot) = self.change_tracker.write().create_snapshot(task_id.clone(), working_dir.clone()).await {
            tracing::info!("[AgentRuntime] Created snapshot for task: {}", task_id);
        }
        
        // Add to active tasks
        self.active_tasks.write().insert(task_id.clone(), task.clone());
        ...
        while attempt <= self.max_retries {
            if attempt > 0 {
                ...
                if let Some(correction) = self.analyze_error_and_suggest_fix(&task, &error_msg).await {
                    self.emit_reasoning(
                        &task_id,
                        format!("Correction plan: {}", correction),
                        None,
                    );
            ...
            match result {
                Ok(value) => {
                    ...
                    if attempt > 0 {
                        tracing::info!("[AgentRuntime] Task completed after {} correction attempt(s): {}", attempt, task_id);
                    } else {
                ...
                Err(e) => {
                    ...
                    if attempt > self.max_retries {
                        ...
                        tracing::error!("[AgentRuntime] Task failed after {} attempts: {} - {}", self.max_retries + 1, task_id, error_msg);
    ...
    async fn execute_code_task(&self, task: &Task) -> Result<serde_json::Value> {
        self.emit_reasoning(
            &task.id,
            format!("Detected code-related task: {}. Analyzing requirements and generating code...", task.description),
            None,
        );
    ...
    async fn execute_with_mcp_tools(&self, task: &Task) -> Result<serde_json::Value> {
        ...
        self.emit_reasoning(
            &task.id,
            format!("Found {} MCP tools available. Selecting appropriate tools for task...", tools.len()),
            None,
        );
        ...
        if !relevant_tools.is_empty() {
            self.emit_reasoning(
                &task.id,
                format!("Selected {} relevant tool(s) for execution", relevant_tools.len()),
                None,
            );
    ...
    pub async fn revert_task_changes(&self, task_id: &str) -> Result<Vec<String>, String> {
        ...
        let changes: Vec<Change> = {
            ...
            tracker.get_task_changes(task_id).into_iter().cloned().collect()
        };
        ...
        for change in changes.iter().rev() {
            ...
            match self.revert_change(change).await {
                Ok(id) => {
                    ...
                    self.change_tracker.write().mark_reverted(&change.id).map_err(|e| e.to_string())?;
                }
                Err(e) => {
                    tracing::error!("[AgentRuntime] Failed to revert change {}: {}", change.id, e);
        ...
        match &change.change_type {
            ...
            ChangeType::FileModified => {
                if let (Some(path), Some(before_content)) = (&change.path, &change.before_content) {
                    tokio::fs::write(path, before_content).await.map_err(|e| format!("Failed to restore file: {}", e))?;
            ...
            ChangeType::FileDeleted => {
                if let (Some(path), Some(content)) = (&change.path, &change.before_content) {
                    ...
                    tokio::fs::write(path, content).await.map_err(|e| format!("Failed to restore deleted file: {}", e))?;
            ...
            ChangeType::CommandExecuted { .. } => {
                ...
                tracing::warn!("[AgentRuntime] Cannot revert command execution: {:?}", change.change_type);
            }
            _ => {
                tracing::warn!("[AgentRuntime] Revert not implemented for change type: {:?}", change.change_type);
            }
        }
    ...
    pub fn get_task_change_history(&self, task_id: &str) -> Vec<Change> {
        self.change_tracker.read().get_task_changes(task_id).into_iter().cloned().collect()
    }
    ...
    pub fn emit_todo_update(&self, task_id: &str, todos: Vec<(String, String, String)>) {
        let todo_list: Vec<serde_json::Value> = todos
            .into_iter()
            .map(|(id, content, status)| serde_json::json!({
                "id": id,
                "content": content,
                "status": status,
            }))
