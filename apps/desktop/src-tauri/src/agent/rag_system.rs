...
impl RAGSystem {
    ...
    /// Index a code file
    pub async fn index_code_file(&mut self, file_path: PathBuf, content: String) -> Result<(), String> {
    ...
    async fn parse_code_file(&self, file_path: &PathBuf, content: &str) -> Result<Vec<CodeChunk>, String> {
            ...
            if line == "}" || line == "};" {
                if current_function.is_some() {
                    let chunk = CodeChunk {
                        ...
                        dependencies: self.extract_dependencies(&lines[current_chunk_start..=i].join("\n")),
    ...
    pub fn search_code(&self, query: &str, limit: usize) -> Vec<&CodeChunk> {
        ...
        let mut results: Vec<(&String, &CodeChunk)> = self.code_index
            .iter()
            .filter(|(_, chunk)| {
                chunk.content.to_lowercase().contains(&query_lower)
                    || chunk.function_name.as_ref().map(|n| n.to_lowercase().contains(&query_lower)).unwrap_or(false)
                    || chunk.doc_comment.as_ref().map(|d| d.to_lowercase().contains(&query_lower)).unwrap_or(false)
            })
            .collect();
        ...
        results.into_iter().take(limit).map(|(_, chunk)| chunk).collect()
    }
    ...
    pub fn retrieve_context(&self, task_description: &str, limit: usize) -> RAGContext {
        ...
        // Search experiences
        let experiences: Vec<&Experience> = self.experience_index
            .values()
            .filter(|exp| {
                exp.task_description.to_lowercase().contains(&task_description.to_lowercase())
                    || exp.tags.iter().any(|tag| task_description.to_lowercase().contains(&tag.to_lowercase()))
            })
            .take(limit)
            .collect();
        
        // Search documentation
        let doc_chunks: Vec<&DocChunk> = self.doc_index
            .values()
            .filter(|doc| {
                doc.content.to_lowercase().contains(&task_description.to_lowercase())
                    || doc.title.to_lowercase().contains(&task_description.to_lowercase())
            })
            .take(limit)
            .collect();
    ...
    pub fn store_experience(&mut self, experience: Experience) {
        self.experience_index.insert(experience.id.clone(), experience);
    }
