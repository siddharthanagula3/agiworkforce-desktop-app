...
pub enum ExcelEdit {
    UpdateCell { sheet: String, row: u32, col: u16, value: String },
    InsertRow { sheet: String, row: u32, values: Vec<String> },
    DeleteRow { sheet: String, row: u32 },
    InsertColumn { sheet: String, col: u16, values: Vec<String> },
    DeleteColumn { sheet: String, col: u16 },
    SetFormula { sheet: String, row: u32, col: u16, formula: String },
    UpdateStyle { sheet: String, row: u32, col: u16, bold: Option<bool>, color: Option<String> },
}
...
impl ExcelEditor {
    ...
    pub fn edit_spreadsheet(&self, _file_path: &str, edits: Vec<ExcelEdit>, output_path: &str) -> Result<()> {
        ...
        let mut sheets: std::collections::HashMap<String, Worksheet> = std::collections::HashMap::new();
    ...
    fn apply_edit(
        ...
        match edit {
            ExcelEdit::UpdateCell { sheet, row, col, value } => {
            ...
            ExcelEdit::SetFormula { sheet, row, col, formula } => {
                let worksheet = sheets.entry(sheet.clone()).or_insert_with(|| Worksheet::new());
    ...
    pub fn update_cell(&self, file_path: &str, sheet: &str, row: u32, col: u16, value: &str, output_path: &str) -> Result<()> {
    ...
    pub fn add_row(&self, file_path: &str, sheet: &str, row: u32, values: Vec<String>, output_path: &str) -> Result<()> {
    ...
    pub fn set_formula(&self, file_path: &str, sheet: &str, row: u32, col: u16, formula: &str, output_path: &str) -> Result<()> {
